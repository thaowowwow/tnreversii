  ...
  */
  ...
  socket.on('play_token', (payload) => {
    ...
    if ((typeof username == 'undefined') || (username === null)) {
      response = {};
      response.result = 'fail';
      response.message = 'play_token command did not come from a registered user name';
      socket.emit('play_token_response', response);
      serverLog('play_token command failed', JSON.stringify(response));
      return;
    }
...
/* Check to make sure that the adjacent support in on the board */
    if ((r + dr < 0) || ( r + dr > 7)) {
        return false;
    }
    if ((c + dc < 0) || ( c + dc > 7)) {
...
/* Check to make sure that there is space for a matching color to capture token */
...
function send_game_update(socket, game_id, message) {
  ...
  /* Check to see if the game with the game_id exists */
